...
Chrome / Edge / Safari 书签同步 · 多主题与九种布局 · 动态天气 · Windows 安装向导 · Gitee / GitHub 双源升级。
...
- 9 种页面布局：标准、紧凑、列表、宫格、分组看板、目录树、顶部标签、居中起始页、分类折叠。
- 16 种网站图标风格，以及包含悬浮、翻转、聚焦、漂移和心跳在内的 16 项图标动效。
- 在“外观 → 页面布局”中可调整 60%–120% 的书签密度；缩小卡片时，图标和文字会同步缩小、每行显示更多书签，设置保存在本机。
- 鼠标拖尾与背景特效各提供 16 项选择，并支持单独关闭、大小和数量调节。
- 所有选择都会保存在浏览器本地；关闭“动态效果”后会同时停用背景和图标动画。

在“外观 → 页面布局”切换：**分组看板**将分类并排展示，各组可独立加载更多，选择“全部”可总览所有分类；**目录树**将多级分类收进一个可展开、收起的侧栏。**顶部标签**将一级分类横排，使用面包屑返回上级、下级分类按钮继续浏览；**居中起始页**将时钟、天气和搜索居中，下方用快捷图标展示当前分类，选择“全部”时展示分类入口。**分类折叠**将分类纵向排列，每组独立展开、收起和加载更多，搜索或切换分类时重新展开。切换布局保留当前分类与搜索，继续使用原主题、图标和天气效果。
...
点击右上角“同步书签”，选择浏览器，再点击“确认同步”。Windows 会检索有有效书签的 Chrome、Edge、Brave、Vivaldi、Opera、Opera GX、QQ、360 极速/极速 X、搜狗高速、夸克和 UC 浏览器；macOS 支持 Chrome、Safari。其他浏览器或使用专有收藏格式的版本，请先在书签管理中导出 HTML 文件，再选择“其他浏览器”并在弹出的窗口中选取该文件。默认识别当前浏览器类型，不支持或无法识别时需要手动选择，不会擅自切换同步来源。
